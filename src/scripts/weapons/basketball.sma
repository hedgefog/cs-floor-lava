...
#define WEAPON_NAME WEAPON(Basketball)
...
public plugin_precache() {
  Asset_Precache(ASSET_LIBRARY, ASSET(BasketballViewModel), g_szViewModel, charsmax(g_szViewModel));
  Asset_Precache(ASSET_LIBRARY, ASSET(BasketballPlayerModel), g_szPlayerModel, charsmax(g_szPlayerModel));
  Asset_Precache(ASSET_LIBRARY, ASSET(BasketballWorldModel), g_szWorldModel, charsmax(g_szWorldModel));
  ...
  // CW_Ammo_Register("basketball", 3);
}

public plugin_init() {
  register_plugin(WEAPON_PLUGIN(Basketball), FLOORLAVA_VERSION, "Hedgehog Fog");
}
...
@Weapon_Create(const this) {
  ...
  CW_SetMember(this, CW_Member_iId, FloorLava_WeaponId_Basketball);
  CW_SetMember(this, CW_Member_iPrimaryAmmoType, 4);
  // CW_SetMemberString(this, CW_Member_szPrimaryAmmo, "basketball");
  CW_SetMember(this, CW_Member_iSlot, 3);
  CW_SetMember(this, CW_Member_iPosition, 5);
  CW_SetMemberString(this, CW_Member_szIcon, "basketball");

  CW_SetMember(this, Weapon_BaseThrowable_Member_flThrowForce, 920.0);
}
...
@Weapon_SpawnProjectile(const this) {
  ...
  new pGrenade = CE_Create(ENTITY(Basketball), vecSrc);
